...
import pytest
import mock
from ansible_collections.amarao.ip.plugins.modules import (
...
@pytest.mark.parametrize(
    "input,output",
    [("\n", []), ("", []), ("\n".join([IF1, IF2]), [IF1_data, IF2_data])],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "broadcast",
            "FA:FA:FA:FA:FA:FA",
            "ip link set dev eth0 broadcast fa:fa:fa:fa:fa:fa",
        ),  # noqa
        (
            "address",
            "BB:BB:BB:BB:BB:BB",
            "ip link set dev eth0 address bb:bb:bb:bb:bb:bb",
        ),  # noqa
